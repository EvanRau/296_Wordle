...
def compare(answer, guess):
    res = list('')
    exi = ''
    for i in range(5): # Words can only be length 5
        if guess[i] == answer[i]:
            res += '2'
            exi += guess[i]
            if answer.count(guess[i]) < exi.count(guess[i]): #if # of marked letters exceeds # in word
                x = 0; #dummy variable to only remove one excess '1'
                for j in range(i):
                    if res[j] == '1' and guess[i] == exi[j] and x == 0:
                        res[j] = '0' #finds excess 1 in list and swaps it to 0
                        x +=1
        elif guess[i] in answer and answer.count(guess[i]) > exi.count(guess[i]):
            res += '1'
            exi += guess[i]
...
def partition(guess, wordlist):
    dictionary = {}
    for answer in wordlist:
        pattern = compare(answer, guess)
...
def bestGuess(wordlist):
    ...
    for guess in wordlist:
        num = 0
        dictionary = partition(guess, wordlist)
        for x in dictionary.keys():
            num += len(dictionary[x])*math.log(len(dictionary[x]))
        num = num/len(wordlist)
        if num < temp:
            temp = num
            best = guess
            print(num)
    print("The best guess is:", best)
    return best
            
def placeholder(guess):
    num = 0
    dictionary = partition(guess, wordlist)
    for x in dictionary.keys():
        num += len(dictionary[x])*math.log(len(dictionary[x]))
...
def fetchWord(guess, pattern):
    dictionary = partition(guess, wordlist)
    print(dictionary[pattern])
    best = bestGuess(dictionary[pattern])
    return best

##################################################################
def partitionPair(guess1, guess2, wordlist):
    d1 = {}
    d2 = {}
    d1 = partition(guess1, wordlist)
    for key in d1:
        d2[key] = partition(guess2, d1[key])
    return d2
    
def BestPair(wordlist):
    temp = 10000000000000
    best = ''
    prog = 0
    for guess1 in wordlist:
        prog += 1
        print("Word:", guess1, " Progress:", prog, "/", len(wordlist))
        d1 = partition(guess1, wordlist)
        for guess2 in wordlist:
            num = 0
            dictionary = {}
            for key in d1:
                dictionary[key] = partition(guess2, d1[key])
            for x in dictionary.keys():
                for y in dictionary[x].keys():
                    num += len(dictionary[x][y])*math.log(len(dictionary[x][y]))
            num = num/len(wordlist)
            ...
            if num < temp:
                ...
                best = guess1
                print(num)
    print("The best guess is", best, "with quality", temp)
    return best
#Trice#


def guessQual(guess, wordlist):
    num = 0
    dictionary = partition(guess, wordlist)
    for x in dictionary.keys():
        num += len(dictionary[x])*math.log(len(dictionary[x]))
...
def guessQualPair(guess, wordlist):
    d1 = partition(guess, wordlist)
    for guess2 in wordlist:
        num = 0
        dictionary = {}
        for key in d1:
